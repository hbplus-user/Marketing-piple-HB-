...
export default function ReviewFeedbackModal({ open, requestId }: { open: boolean; requestId?: string }) {
  const { requests, closeModal, approveRequest, requestChanges, removeCreatorFromApproval, addComment, updateRequest, editSubmission, currentUser, openModal, users } = useApp();
  ...
  // Plain comment — posts to the feed without requesting changes or approving,
  // so a reviewer can leave a remark without moving the task's status.
  const handleSendComment = () => {
    if (!comment.trim() || refLinkInvalid) return;
    addComment(req.id, comment.trim(), refLinkTrimmed || undefined);
    setComment('');
    setRefLink('');
  };
      ...
      <div className="flex flex-col md:flex-row h-auto md:h-[75vh]">
        ...
        <div className="w-full md:flex-[2] flex flex-col">
                      ...
                      <div className="space-y-2">
                        ...
                        {editLinks.map((url, i) => (
                          <div key={i} className="flex items-center gap-1.5 px-2.5 py-1.5 bg-white rounded-lg border border-[#f0ddd5]">
                            <ExternalLink size={10} className="text-[#a9674d] flex-shrink-0" />
          ...
          <div className="px-5 py-4 border-t border-gray-100 space-y-2.5">
            ...
            {/* Send — posts a plain comment without requesting changes or approving */}
            <button
              onClick={handleSendComment}
              disabled={!comment.trim() || refLinkInvalid}
              className="w-full flex items-center justify-center gap-1.5 py-2 text-xs font-medium border border-gray-200 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed text-gray-700 rounded-lg transition-colors"
            >
              <Send size={12} />
              Send
            </button>
